...
var Debug = {
    ...
    Draw() {
        if (this.cvs !== undefined) {
            let runtime = "runtime in sec: " + Browser.runTime / 1000,
                fps = " fps: " + Math.floor((1 / Browser.deltaTime) * 1000),
                size = "width: " + Browser.width + " height: " + Browser.height + fps,
                mspf = "mspf: " + Browser.deltaTime;
    ...
    Log(log) { this.Check(log, 0, "log"); },
    Info(info) { this.Check(info, 1, "info"); },
    Warn(warn) { this.Check(warn, 2, "warn"); },
    Debug(debug) { this.Check(debug, 3, "dbug"); },
    Error(error) { this.Check(error, 4, "err"); }
}